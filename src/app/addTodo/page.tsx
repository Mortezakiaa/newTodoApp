    ...
    <>
      ...
      <div className='px-3 mt-2'>
        ...
        <div className="flex items-center justify-between px-2 mt-2 bg-teal-600 rounded-md w-[200px]">
          <RadioButton Checked={state.status === 'DailyTodo'} ChangeStatus={e=> ChangeStatus(e)} Value={'todo'}/>
          <BsAlignStart color="white"/>
        </div>
        <div className="flex items-center justify-between px-2 mt-2 bg-cyan-800 rounded-md w-[200px]">
          <RadioButton Checked={state.status === 'WeeklyTodo'} ChangeStatus={e=> ChangeStatus(e)} Value={'in progress'}/>
          <FiSettings color="white"/>
        </div>
        <div className="flex items-center justify-between px-2 mt-2 bg-green-700 rounded-md w-[200px]">
          <RadioButton Checked={state.status === 'MonthlyTodo'} ChangeStatus={e=> ChangeStatus(e)} Value={'done'}/>
          <AiOutlineFileSearch color="white" />
        </div>
        <div>
          <button type="button" onClick={SaveTodos} className="mt-2 text-white bg-slate-500 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm w-full sm:w-auto px-5 py-2.5 text-center">Add Todo</button>
        </div>
      </div>
    </>
  )
